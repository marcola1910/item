package org.exnihilo.Item.test;
...
public class Main {
	...
	public static void main(String[] args) {
		...
		// 1 - configure the repositories locator for mock
		ItemSys mItem = new ItemSys();
		...
		// 2 - configure the services locator for mock

		// cuando termine el model lo hago, porque solo va a servir para la
		// comunicacion con WEB.
		// las paginas conocen solo los servicios, los servicios conocen al
		// modelo.
		...
		// 3 - pop up the Masters: Equipo, miembroequipo, puesto, usuario
		...
		MiembroEquipoDTO mMarc = DTOFactory.getInstance().createMiembroDTO(
				"Marc Alcantara", "mm@company.com", nAnalista, null);
		...
		MiembroEquipoDTO mJose = DTOFactory.getInstance().createMiembroDTO(
				"Jose Perez", "mm@company.com", nAnalistaInfra, null);
		...
		MiembroEquipoDTO mJuan = DTOFactory.getInstance().createMiembroDTO(
				"Juan", "mm@company.com", nHelpDesk, null);
		...
		// 2 mi itens proyecto logistica de desarrollo
		for (int i = 1; i <= 2000000; i++) {
			...
			ItemDTO _nItem = DTOFactory.getInstance().createItemDTO(
					...
					mLogistica
							.getEquipo()
							.getMiembros()
							.get(rand.nextInt(mLogistica.getEquipo() //Agarra un miembro aleatorio del equipo
...
/*		// 1 mi itens proyecto logistica de bug fixing
		for (int i = 1; i <= 1000000; i++)
			mItem.getItens().add(
					new Item("titulo INFRA item #" + i,
							"descripcion descripcion item INFRA #" + i,
							mItemTypeIF, mProyecto));*/

		// changing the new workflow changing some states sequence

		// System.out.println(SizeOf.deepSizeOf(mProyectoPP)); //this will print
		// the object size in bytes
		
		
		UsuarioDTO _user =  ServiceLocator.getInstance().getUserService().checkLogin("adm@nihil.com", "adm");
		System.out.println("Tchan Autenticado::" + _user.isLoginCHecked());
		
		_user =  ServiceLocator.getInstance().getUserService().checkLogin("adm2@nihil.com", "adm");
		System.out.println("Tchan Autenticado::" + _user.isLoginCHecked());
		
		_user =  ServiceLocator.getInstance().getUserService().checkLogin("adm@nihil.com", "adm1");
		System.out.println("Tchan Autenticado::" + _user.isLoginCHecked());
